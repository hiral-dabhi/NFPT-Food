    ...
    <div class="main-content group-data-[sidebar-size=sm]:ml-[70px]">
        <div class="page-content dark:bg-zinc-700">
            <div class="container-fluid px-[0.625rem]">
                ...
                <!-- page title -->
                <x-page-title title="Create Ipv6" pagetitle="Ipv6" />
                            ...
                            <div class="bg-white sm:p-6 shadow sm:rounded-md">
                                <div class="grid grid-cols-6 gap-6">
                                    <div class="col-span-6 sm:col-span-4">
                                        <label for="pool_name"
                                            class="block mb-2 font-medium text-gray-700 dark:text-gray-100">Pool
                                    ...
                                    <div class="col-span-6 sm:col-span-4">
                                        <label for="network"
                                            class="block mb-2 font-medium text-gray-700 dark:text-gray-100">Network<span
                                                class="text-sm text-red-600">*</span></label>
                                        <input type="text" name="network" id="network" placeholder="Enter Network"
                                            class="w-full placeholder:text-13 text-13 py-1.5 rounded border-gray-100 focus:border focus:border-violet-50 focus:ring focus:ring-violet-500/20  dark:bg-zinc-700/50 dark:border-zinc-600 dark:placeholder:text-zinc-300 placeholder:text-gray-400 dark:text-zinc-100"
                                            value="{{ old('network') }}">
                                        @error('email')
                                            <p class="text-sm text-red-600">{{ $message }}</p>
                                    ...
                                    <div class="col-span-6 sm:col-span-4">
                                        <label for="prefix_length_id"
                                            class="block mb-2 font-medium text-gray-700 dark:text-gray-100">Prefix
                                            Length<span class="text-sm text-red-600">*</span></label>
                                        <select name="prefix_length_id" id="role"
                                            class="w-full placeholder:text-13 text-13 py-1.5 rounded border-gray-100 focus:border focus:border-violet-50 focus:ring focus:ring-violet-500/20  dark:bg-zinc-700/50 dark:border-zinc-600 dark:placeholder:text-zinc-300 placeholder:text-gray-400 dark:text-zinc-100">
                                            <option value="">Select Prefix Length</option>
                                            @foreach ($prefix as $key => $value)
                                                <option value="{{ $value->id }}"
                                                    {{ old('prefix_length_id') == $value->id ? 'selected' : '' }}>
                                                    {{ $value->title }}</option>
                                            @endforeach
                                        </select>

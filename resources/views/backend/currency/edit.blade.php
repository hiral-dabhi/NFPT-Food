    ...
    <div class="main-content group-data-[sidebar-size=sm]:ml-[70px]">
        <div class="page-content dark:bg-zinc-700">
            <div class="container-fluid px-[0.625rem]">
                ...
                <!-- page title -->
                <x-page-title title="Edit Currency" pagetitle="Currency" />
                                    ...
                                    <div class="col-span-12 lg:col-span-6">
                                        <div class="mb-4">
                                            ...
                                            <select name="country_id" id="country_id"
                                                ...
                                                @foreach ($countryList as $key => $value)
                                                    <option value="{{ $key }}"
                                                        {{ $currency->country_id == $key ? 'selected' : '' }}>
                                                        {{ $value }}</option>
                                                @endforeach
                                            </select>
                                            @error('country_id')
                                        ...
                                        <div class="mb-4">
                                            <label for="sign_image"
                                                class="block mb-2 font-medium text-gray-700 dark:text-gray-100">Sign
                                                Image</label>
                                            ...
                                            @if ($currency->image)
                                                <div class="mb-2">
                                                    <img src="{{ asset('currency/' . $currency->image) }}" alt="Sign Image"
                                            ...
                                            <div class="mt-1 flex items-center">
                                                <div class="flex items-center" style="margin-right: 10px">
                                                    <input type="radio" id="status1" name="status" value="1"
                                                        {{ old('status', $currency->status) === '1' ? 'checked' : '' }}
                                                ...
                                                <div class="flex items-center">
                                                    <input type="radio" id="status2" name="status" value="0"
                                                        {{ old('status', $currency->status) === '0' ? 'checked' : '' }}
                                                        class="form-radio h-4 w-4 text-indigo-600 transition duration-150 ease-in-out">
                                                    <label for="status2"

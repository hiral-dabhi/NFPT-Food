    ...
    <div class="main-content group-data-[sidebar-size=sm]:ml-[70px]">
        <div class="page-content dark:bg-zinc-700">
            <div class="container-fluid px-[0.625rem]">
                <x-page-title title="Add Sub Category" pagetitle="Sub Category" />

                <div class="grid grid-cols-1 mt-3">
                    <div class="card dark:bg-zinc-800 dark:border-zinc-600">
                        <div class="card-body">
                            <form method="POST" action="{{ route('subCategory.store') }}" class="subCategory-create-form">
                                @csrf
                                            ...
                                            <div class="mt-1 flex items-center">
                                                ...
                                                <div class="flex items-center">
                                                    <input type="radio" id="status2" name="status" value="1"
                                                        ...
                                                        class="form-radio h-4 w-4 text-indigo-600 transition duration-150 ease-in-out">
                                                    <label for="status2"
                                                        class="ml-2 block text-sm leading-5 text-gray-700">In-Active</label>
                                                </div>
                                            </div>
                                            @error('status')
                                                <p class="text-sm text-red-600">{{ $message }}</p>
...
@section('scripts')
    ...
    <script>
        var getCountryname = "{{ route('subCategory.getCountryName') }}";
        $(function() {
            subCategory.create();
        });
    </script>
